            ...
            for col in range(self._y, self._y + 1):
                if (self.killedByEnms):
                    ...
                    if (board._bufferboard[row, col] == b"G"):
                        for i in board.enms:
                            if ((self._y >= i._y and self._endy >= i._y) or (self._endy <= i._endy and self._y <= i._endy)):
                                board._bufferboard[i._x:i._endx,
                                                   i._y:i._endy] = ""
                                board.enms.remove(i)
                                self.powerup1 = True
...
def reset(self, board, ch):
    ...
    board.__init__(board.height, board.width, board.endlevel_x, board.endlevel_endx,
                   board.endlevel_y, board.endlevel_endy, board.resen, board.resob, board.coins, board.player)
